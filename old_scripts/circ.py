...
K_DISTANCE = 1.0          # Distance correction gain for bank angle (reduced)
K_DERIVATIVE = 0.5        # Derivative gain for damping distance error changes
...
def glide_to_waypoint(vehicle, waypoint):
    """Control glide and circling to waypoint using bank angle control."""
    vehicle.mode = VehicleMode("GUIDED")
    while vehicle.mode != "GUIDED":
        time.sleep(1)
    ...
    last_distance = None  # For derivative term
    ...
    while current_alt > IDEAL_HEIGHT or current_speed > IDEAL_SPEED:
        ...
        if last_distance is not None:
            distance_rate = (distance - last_distance) / CONTROL_FREQ
            bank_damping = K_DERIVATIVE * distance_rate
            bank_correction -= bank_damping
