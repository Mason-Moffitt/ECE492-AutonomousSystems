...
def glide_to_waypoint(vehicle, waypoint):
    ...
    while True:
        ...
        desired_roll = -K_ROLL * heading_error  # Inverted for CCW
        desired_roll = max(min(desired_roll, MAX_ROLL), -MAX_ROLL)
        ...
        if time.time() - last_print_time > 1:
            print(f"CCW Circling - Dist: {distance:.1f}m, Alt: {current_alt:.1f}m, Yaw: {current_yaw:.1f}°, Desired: {desired_heading:.1f}°, Error: {heading_error:.1f}°, RelBearing: {relative_bearing:.1f}°, Roll: {desired_roll:.1f}°")
            last_print_time = time.time()
        time.sleep(CONTROL_FREQ)
