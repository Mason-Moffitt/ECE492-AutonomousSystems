...
K_P = 0.1                 # Pitch gain
K_DISTANCE = 1.0          # Distance correction gain
MAX_BANK_DEVIATION = 15.0 # Maximum deviation from ideal bank angle (degrees)
...
def glide_to_waypoint(vehicle, waypoint):
    ...
    while True:
        ...
        if abs(heading_error) < 5 or current_alt <= IDEAL_HEIGHT:
            logging.info("Starting CCW circling...")
            print("Starting CCW circling...")
            break

        # Right turn to align heading
        desired_roll = K_ROLL * heading_error
        desired_roll = max(min(desired_roll, MAX_ROLL), -MAX_ROLL)
        ...
        if time.time() - last_print_time > 1:
            print(f"Aligning - Dist: {distance:.1f}m, Alt: {current_alt:.1f}m, Heading Error: {heading_error:.1f}°")
            last_print_time = time.time()
        time.sleep(CONTROL_FREQ)
    ...
    while current_alt > IDEAL_HEIGHT:
        ...
        # Limit distance correction to MAX_BANK_DEVIATION
        distance_correction = max(min(K_DISTANCE * error_distance, MAX_BANK_DEVIATION), -MAX_BANK_DEVIATION)
...
def main():
    ...
    try:
        ...
        start_location = get_startCord_input()
