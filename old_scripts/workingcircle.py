...
def glide_to_waypoint(vehicle, waypoint):
    ...
    with open('circling_log.txt', 'a') as f:
        f.write("Time,Distance,Bank,Error_Distance,Heading_Error,Tangent_Direction,Speed,Circle_Direction\n")
    while current_alt > IDEAL_HEIGHT or current_speed > IDEAL_SPEED:
        ...
        bearing_to_plane = calculate_bearing(waypoint, current_loc)  # From waypoint to plane
        if circle_direction == 'CW':
            tangent_direction = (bearing_to_plane - 90) % 360  # Tangent is 90° left of radial line
        else:  # CCW
            tangent_direction = (bearing_to_plane + 90) % 360  # Tangent is 90° right of radial line
        ...
        with open('circling_log.txt', 'a') as f:
            f.write(f"{time.time()},{distance},{desired_bank},{error_distance},{heading_error},{tangent_direction},{current_speed},{circle_direction}\n")
