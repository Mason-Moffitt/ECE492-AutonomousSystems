...
def glide_to_waypoint(vehicle, waypoint):
        ...
        while True:
            ...
            # Calculate desired heading for CCW circling
            bearing = calculate_bearing(current_loc, waypoint)
            desired_heading = (bearing + 90) % 360  # CCW is 90° left of bearing to waypoint
            angular_error = (desired_heading - current_yaw + 180) % 360 - 180  # Normalize to [-180, 180)
            desired_roll = K_ROLL * angular_error
            desired_roll = max(min(desired_roll, MAX_ROLL), -MAX_ROLL)
    
            # Dynamic pitch control (same as glide phase)
            desired_descent_rate = ground_speed / GLIDE_RATIO if ground_speed > 0 else 0
            filtered_descent_rate = ALPHA * last_descent_rate + (1 - ALPHA) * actual_descent_rate
            last_descent_rate = filtered_descent_rate
            error_descent = desired_descent_rate - filtered_descent_rate
            ...
            # Debug output
            if time.time() - last_print_time > 1:
                print(f"Circling - Dist: {distance:.1f}m, Alt: {current_alt:.1f}m, Angular Error: {angular_error:.1f}°, Roll: {desired_roll:.1f}°, Pitch: {desired_pitch:.1f}°")
                last_print_time = time.time()
            time.sleep(CONTROL_FREQ)
