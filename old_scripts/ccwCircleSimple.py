...
FIXED_ROLL = 15.0         # Fixed roll angle for CCW circling when outside radius (degrees)
DISTANCE_TOLERANCE = 2.0  # Tolerance for distance error to prevent chattering (meters)
...
def glide_to_waypoint(vehicle, waypoint):
    ...
    while True:
        ...
        if distance <= IDEAL_RADIUS:
            ...
            while time.time() - start_circle_time < 4.0:
                ...
                desired_bank = k_heading_current * heading_error  # Positive bank for CCW
    ...
    with open('circling_log.txt', 'a') as f:
        f.write("Time,Distance,Bank,Error_Distance,Speed,Circle_Direction,Heading,Bearing_to_Plane\n")
    while current_alt > IDEAL_HEIGHT or current_speed > IDEAL_SPEED:
        ...
        # Fixed roll when outside radius with tolerance
        desired_bank = FIXED_ROLL if error_distance > DISTANCE_TOLERANCE else 0.0  # Roll left (positive) for CCW if too far
        ...
        # Log data
        with open('circling_log.txt', 'a') as f:
            f.write(f"{time.time()},{distance},{desired_bank},{error_distance},{current_speed},{circle_direction},{current_yaw},{bearing_to_plane}\n")

        if time.time() - last_print_time > 0.5:
            print(f"Circling - Distance: {distance:.1f}m, Alt: {current_alt:.1f}m, Bank: {desired_bank:.1f}°, Heading Error: {0.0:.1f}°, Speed: {current_speed:.1f}m/s, Heading: {current_yaw:.1f}°, Bearing: {bearing_to_plane:.1f}°, Direction: {circle_direction}")
            last_print_time = time.time()
