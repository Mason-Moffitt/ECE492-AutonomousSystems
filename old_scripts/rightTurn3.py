...
K_ROLL = 1.0              # Roll gain for all phases
K_P = 0.1                 # Pitch gain
K_DISTANCE = 0.6          # Distance correction gain
MAX_BANK_DEVIATION = 0.2 * CIRCLE_RADIUS  # Max deviation from ideal bank (meters, relative to radius)
...
def glide_to_waypoint(vehicle, waypoint):
    ...
    # Heading alignment phase (align so waypoint is on left for CCW circling, shortest turn)
    while True:
        current_loc = vehicle.location.global_frame
        current_alt = vehicle.location.global_relative_frame.alt
        current_speed = vehicle.airspeed
        ...
        # Calculate desired heading (waypoint 90° to left for CCW circling)
        bearing_to_plane = calculate_bearing(waypoint, current_loc)
        desired_heading = (bearing_to_plane - 90) % 360  # Waypoint on left
        # Compute heading error for shortest turn
        heading_error = (desired_heading - current_yaw + 360) % 360 - 180

        # Check relative bearing to confirm waypoint is on left
        relative_bearing = (bearing_to_plane - current_yaw + 360) % 360
        if relative_bearing > 180:
            relative_bearing -= 360  # Normalize to [-180, 180]
        alignment_ok = abs(heading_error) < 5 and abs(relative_bearing - (-90)) < 10

        if alignment_ok or current_alt <= IDEAL_HEIGHT:
            logging.info("Starting CCW circling...")
            print("Starting CCW circling...")
            break

        # Turn shortest way to align with desired heading
        desired_roll = K_ROLL * heading_error
        desired_roll = max(min(desired_roll, MAX_ROLL), -MAX_ROLL)
        ...
        if time.time() - last_print_time > 1:
            print(f"Aligning - Dist: {distance:.1f}m, Alt: {current_alt:.1f}m, Yaw: {current_yaw:.1f}°, Bearing: {bearing_to_plane:.1f}°, Target: {desired_heading:.1f}°, Error: {heading_error:.1f}°, RelBearing: {relative_bearing:.1f}°, Roll: {desired_roll:.1f}°")
            last_print_time = time.time()
        time.sleep(CONTROL_FREQ)
    ...
    while current_alt > IDEAL_HEIGHT:
        ...
        default_bank = degrees(atan2(filtered_speed**2, g * CIRCLE_RADIUS))
        error_distance = distance - CIRCLE_RADIUS
        # Convert MAX_BANK_DEVIATION (meters) to degrees
        max_bank_deviation_deg = degrees(atan2(filtered_speed**2, g * (CIRCLE_RADIUS + MAX_BANK_DEVIATION))) - default_bank
        distance_correction = max(min(K_DISTANCE * error_distance, max_bank_deviation_deg), -max_bank_deviation_deg)
        desired_bank = -(default_bank + distance_correction)
        ...
        if time.time() - last_print_time > 0.5:
            print(f"Circling - Dist: {distance:.1f}m, Alt: {current_alt:.1f}m, Bank: {desired_bank:.1f}°, Dist Correction: {distance_correction:.1f}°")
            last_print_time = time.time()
        time.sleep(CONTROL_FREQ)
