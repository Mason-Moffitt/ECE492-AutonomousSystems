...
K_DISTANCE = 1.0          # Distance correction gain for bank angle
...
def glide_to_waypoint(vehicle, waypoint):
    ...
    with open('circling_log.txt', 'a') as f:
        f.write("Time,Distance,Bank,Error_Distance,Distance_Rate,Speed\n")
    while current_alt > IDEAL_HEIGHT or current_speed > IDEAL_SPEED:
        ...
        if last_distance is not None:
            ...
            bank_correction -= K_DERIVATIVE * distance_rate
        ...
        with open('circling_log.txt', 'a') as f:
            f.write(f"{time.time()},{distance},{desired_bank},{error_distance},{distance_rate},{current_speed}\n")
