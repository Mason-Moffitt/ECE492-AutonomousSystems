...
K_DISTANCE = 0.5          # Distance correction gain for bank angle (conservative)
...
def glide_to_waypoint(vehicle, waypoint):
    """Control glide and circling to waypoint using bank angle to steer to 100m radius."""
    vehicle.mode = VehicleMode("GUIDED")
    while vehicle.mode != "GUIDED":
        time.sleep(1)
    ...
    with open('circling_log.txt', 'a') as f:
        f.write("Time,Distance,Bank,Error_Distance,Speed,Circle_Direction\n")
    while current_alt > IDEAL_HEIGHT or current_speed > IDEAL_SPEED:
        ...
        desired_bank = K_DISTANCE * error_distance  # Positive to turn toward waypoint, negative to turn away

        # Apply direction multiplier for CW/CCW
        desired_bank = desired_bank * direction_multiplier
        ...
        with open('circling_log.txt', 'a') as f:
            f.write(f"{time.time()},{distance},{desired_bank},{error_distance},{current_speed},{circle_direction}\n")

        if time.time() - last_print_time > 1:
            print(f"Circling - Distance: {distance:.1f}m, Alt: {current_alt:.1f}m, Bank: {desired_bank:.1f}°, Direction: {circle_direction}")
            last_print_time = time.time()
