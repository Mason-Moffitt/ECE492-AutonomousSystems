...
K_DISTANCE = 0.4          # Distance correction gain for bank angle
...
DIRECTION_CONFIRM_TIME = 2.0  # Time to wait before determining direction (seconds)
...
def glide_to_waypoint(vehicle, waypoint):
    """Control glide and circling to waypoint with simplified delayed direction determination."""
    vehicle.mode = VehicleMode("GUIDED")
    while vehicle.mode != "GUIDED":
        time.sleep(1)
    ...
    while True:
        ...
        if distance <= IDEAL_RADIUS:
            print(f"Within {IDEAL_RADIUS}m. Starting circling with distance-based bank control.")
    ...
    with open('circling_log.txt', 'a') as f:
        f.write("Time,Distance,Bank,Error_Distance,Heading_Error,Tangent_Direction,Bearing_to_Plane,Current_Yaw,Heading_Diff,Control_Mode,Speed,Circle_Direction,Direction_Multiplier\n")
    ...
    while current_alt > IDEAL_HEIGHT or current_speed > IDEAL_SPEED:
        ...
        heading_diff = (current_yaw - bearing_to_plane + 360) % 360

        # Determine circling direction after wait period
        if circle_direction is None and current_time - circling_start_time >= DIRECTION_CONFIRM_TIME:
            circle_direction = 'CCW' if 0 <= heading_diff < 180 else 'CW'
            direction_multiplier = 1 if circle_direction == 'CW' else -1
            print(f"Circling direction set to {circle_direction}, heading_diff: {heading_diff:.1f}°, yaw: {current_yaw:.1f}°, bearing_to_plane: {bearing_to_plane:.1f}°")

        # Bank angle control
        if circle_direction is None:
            # Initial distance-based control (first 2 seconds)
            error_distance = distance - CIRCLE_RADIUS
            desired_bank = -K_DISTANCE * error_distance  # Negative: left bank to widen, right to tighten
            heading_error = 0.0  # For logging
            tangent_direction = 0.0  # For logging
            control_mode = 0
        else:
            # Full control with heading and distance correction
            tangent_direction = (bearing_to_plane + 90) % 360 if circle_direction == 'CCW' else (bearing_to_plane - 90) % 360
        ...
        with open('circling_log.txt', 'a') as f:
            f.write(f"{current_time},{distance},{desired_bank},{error_distance},{heading_error},{tangent_direction},{bearing_to_plane},{current_yaw},{heading_diff},{control_mode},{current_speed},{circle_direction or 'None'},{direction_multiplier}\n")

        if current_time - last_print_time > 1:
            status = "Initial circling (distance-based)" if circle_direction is None else "Full control"
            print(f"Circling ({status}) - Distance: {distance:.1f}m, Alt: {current_alt:.1f}m, Bank: {desired_bank:.1f}°, Heading Error: {heading_error:.1f}°, Direction: {circle_direction or 'Pending'}")
