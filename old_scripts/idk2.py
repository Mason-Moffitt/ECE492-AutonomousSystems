...
IDEAL_RADIUS = 100.0      # Distance to start CCW circling (meters)
CIRCLE_RADIUS = 75.0      # Desired circling radius (meters)
IDEAL_HEIGHT = 15.0       # Height to start final approach (meters)
K_ROLL = 0.8              # Roll gain
K_P = 0.1                 # Pitch gain
K_DISTANCE = 0.4          # Distance correction gain for circling
...
def glide_to_waypoint(vehicle, waypoint):
    ...
    while True:
        ...
        if time.time() - last_print_time > 1:
            print(f"Glide - Dist: {distance:.1f}m, Alt: {current_alt:.1f}m, Yaw: {current_yaw:.1f}°, Bearing: {bearing:.1f}°, Error: {heading_error:.1f}°, Roll: {desired_roll:.1f}°, Pitch: {desired_pitch:.1f}°")
            last_print_time = time.time()
        time.sleep(CONTROL_FREQ)
    ...
    while True:
        ...
        # Adjust roll for radius control
        error_distance = distance - CIRCLE_RADIUS
        roll_adjustment = K_DISTANCE * error_distance
        desired_roll = -K_ROLL * heading_error + roll_adjustment
        desired_roll = max(min(desired_roll, MAX_ROLL), -MAX_ROLL)
        ...
        if current_alt <= IDEAL_HEIGHT:
            ...
            if not (-100 < relative_bearing < -80):
                logging.warning(f"CCW circling not achieved (RelBearing: {relative_bearing:.1f}°)")
                print(f"Warning: CCW circling not achieved (RelBearing: {relative_bearing:.1f}°)")
        ...
        if time.time() - last_print_time > 1:
            print(f"CCW Circling - Dist: {distance:.1f}m, Alt: {current_alt:.1f}m, Yaw: {current_yaw:.1f}°, Desired: {desired_heading:.1f}°, Error: {heading_error:.1f}°, RelBearing: {relative_bearing:.1f}°, Roll: {desired_roll:.1f}°, Pitch: {desired_pitch:.1f}°")
            last_print_time = time.time()
        time.sleep(CONTROL_FREQ)
