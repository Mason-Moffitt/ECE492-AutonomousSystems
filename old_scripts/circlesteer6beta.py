...
MIN_VELOCITY_MAG = 5.0    # Minimum velocity magnitude for direction detection (m/s)
...
def get_velocity_bearing(velocity):
    """Calculate bearing of velocity vector (degrees)."""
    if velocity is None:
        return None
    # velocity[0] = north, velocity[1] = east
    magnitude = sqrt(velocity[0]**2 + velocity[1]**2)
    if magnitude < MIN_VELOCITY_MAG:
        return None
    return (degrees(atan2(velocity[1], velocity[0])) + 360) % 360

def determine_circle_direction(waypoint, current_loc, velocity, current_yaw):
    """Determine circling direction (CW/CCW) based on velocity vector."""
    bearing_to_plane = calculate_bearing(waypoint, current_loc)
    velocity_bearing = get_velocity_bearing(velocity)
    
    if velocity_bearing is not None:
        # Angle difference between velocity and radial bearing
        angle_diff = (velocity_bearing - bearing_to_plane + 360) % 360
        # CCW: velocity leads radial by ~90° (0 to 180)
        # CW: velocity lags radial by ~90° (180 to 360)
        direction = 'CCW' if 0 <= angle_diff < 180 else 'CW'
        print(f"Circle direction set to {direction} based on velocity (angle_diff: {angle_diff:.1f}°)")
        return direction, 1 if direction == 'CW' else -1
    else:
        # Fallback to heading-based direction
        heading_error = (bearing_to_plane - current_yaw + 180) % 360 - 180
        direction = 'CW' if heading_error >= 0 else 'CCW'
        print(f"Circle direction set to {direction} based on heading (fallback)")
        return direction, 1 if direction == 'CW' else -1
...
def glide_to_waypoint(vehicle, waypoint):
    ...
    while True:
        ...
        if distance <= IDEAL_RADIUS:
            # Set direction based on velocity vector
            circle_direction, direction_multiplier = determine_circle_direction(
                waypoint, current_loc, vehicle.velocity, current_yaw
            )
    ...
    with open('circling_log.txt', 'a') as f:
        f.write("Time,Distance,Bank,Error_Distance,Heading_Error,Tangent_Direction,Bearing_to_Plane,Velocity_Bearing,Angle_Diff,Speed,Circle_Direction,Direction_Multiplier\n")
    while current_alt > IDEAL_HEIGHT or current_speed > IDEAL_SPEED:
        ...
        velocity_bearing = get_velocity_bearing(vehicle.velocity) or 0.0
        angle_diff = ((velocity_bearing - bearing_to_plane + 360) % 360) if velocity_bearing else 0.0
        with open('circling_log.txt', 'a') as f:
            f.write(f"{current_time},{distance},{desired_bank},{error_distance},{heading_error},{tangent_direction},{bearing_to_plane},{velocity_bearing},{angle_diff},{current_speed},{circle_direction},{direction_multiplier}\n")
