...
IDEAL_RADIUS = 100.0      # Distance to start alignment (meters)
IDEAL_HEIGHT = 15.0       # Height to start final approach (meters)
K_ROLL = 0.1              # Roll gain
K_P = 0.1                 # Pitch gain
...
def glide_to_waypoint(vehicle, waypoint):
    """Guide drone to waypoint with throttle off, maintaining CCW alignment."""
    vehicle.mode = VehicleMode("GUIDED")
    while vehicle.mode != "GUIDED":
        time.sleep(1)
    ...
    while True:
        ...
        if distance <= IDEAL_RADIUS:
            logging.info("Starting heading alignment...")
            print("Starting heading alignment...")
            break
        ...
        if time.time() - last_print_time > 1:
            print(f"Glide - Dist: {distance:.1f}m, Alt: {current_alt:.1f}m, Yaw: {current_yaw:.1f}°, Bearing: {bearing:.1f}°, Error: {heading_error:.1f}°, Roll: {desired_roll:.1f}°")
            last_print_time = time.time()
        time.sleep(CONTROL_FREQ)

    # Heading alignment phase: Maintain CCW orbit until IDEAL_HEIGHT
    while True:
        current_loc = vehicle.location.global_frame
        current_alt = vehicle.location.global_relative_frame.alt
        current_yaw = vehicle.heading
        ...
        # Calculate desired heading for CCW (waypoint 90° to left)
        bearing_to_plane = calculate_bearing(waypoint, current_loc)
        desired_heading = (bearing_to_plane - 90) % 360
        # Compute heading error for shortest turn
        heading_error = (desired_heading - current_yaw + 360) % 360 - 180

        # Calculate relative bearing to verify waypoint position
        relative_bearing = (bearing_to_plane - current_yaw + 360) % 360
        if relative_bearing > 180:
            relative_bearing -= 360  # Normalize to [-180, 180]

        # Handle edge cases (facing toward/away)
        if abs(relative_bearing) < 5 or abs(relative_bearing - 180) < 5:
            # Bias toward slight right turn to initiate CCW
            heading_error = min(heading_error, 10.0) if heading_error > 0 else max(heading_error, -10.0)

        if current_alt <= IDEAL_HEIGHT:
            # Verify CCW orientation
            if not (-100 < relative_bearing < -80):
                logging.warning(f"CCW alignment not achieved (RelBearing: {relative_bearing:.1f}°)")
                print(f"Warning: CCW alignment not achieved (RelBearing: {relative_bearing:.1f}°)")
            logging.info("Starting final approach...")
            print("Starting final approach...")
            break

        # Turn shortest way to align, invert roll for CCW
        desired_roll = -K_ROLL * heading_error  # Inverted to correct CW to CCW
        desired_roll = max(min(desired_roll, MAX_ROLL), -MAX_ROLL)
        ...
        if time.time() - last_print_time > 1:
            print(f"Aligning - Dist: {distance:.1f}m, Alt: {current_alt:.1f}m, Yaw: {current_yaw:.1f}°, Desired: {desired_heading:.1f}°, Error: {heading_error:.1f}°, RelBearing: {relative_bearing:.1f}°, Roll: {desired_roll:.1f}°")
            last_print_time = time.time()
        time.sleep(CONTROL_FREQ)
...
def main():
    parser = argparse.ArgumentParser(description='Glide to waypoint with CCW alignment.')
