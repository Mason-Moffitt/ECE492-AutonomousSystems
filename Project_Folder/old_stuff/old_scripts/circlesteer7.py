...
K_ROLL = 0.6              # Roll proportional gain for glide phase
K_P = 0.1                 # Pitch proportional gain
K_HEADING = 0.5           # Heading error gain for tangent alignment
K_DISTANCE = 0.4          # Distance correction gain for bank angle (secondary)
...
DIRECTION_CONFIRM_TIME = 1.2  # Time to confirm direction and enable distance correction (seconds)
HEADING_SAMPLES = 3        # Number of heading samples for direction confirmation
HEADING_SAMPLE_INTERVAL = 0.4  # Seconds between heading samples
MIN_YAW_CHANGE = 2.0       # Minimum yaw change per sample (degrees)
...
def confirm_circle_direction(heading_samples):
    """Confirm circling direction (CW/CCW) based on heading changes."""
    if len(heading_samples) < HEADING_SAMPLES:
        return None
    increasing = 0
    decreasing = 0
    for i in range(1, len(heading_samples)):
        delta_yaw = (heading_samples[i][1] - heading_samples[i-1][1] + 360) % 360
        yaw_change = abs(heading_samples[i][1] - heading_samples[i-1][1])
        if delta_yaw < 180 and yaw_change > MIN_YAW_CHANGE:
            increasing += 1
        elif delta_yaw > 180 and yaw_change > MIN_YAW_CHANGE:
            decreasing += 1
    print(f"Heading samples: {[f'{t:.1f}s: {y:.1f}°' for t, y in heading_samples]}, Increasing: {increasing}, Decreasing: {decreasing}")
    if increasing >= 1:  # At least one pair increasing
        return 'CCW'
    if decreasing >= 1:  # At least one pair decreasing
        return 'CW'
    return None
...
def glide_to_waypoint(vehicle, waypoint):
    """Control glide and circling to waypoint using tangent alignment with delayed distance correction."""
    vehicle.mode = VehicleMode("GUIDED")
    while vehicle.mode != "GUIDED":
        time.sleep(1)
    ...
    while True:
        ...
        if distance <= IDEAL_RADIUS:
            # Set initial direction based on heading
            ...
            print(f"Within {IDEAL_RADIUS}m. Starting {circle_direction} circling (initial).")
    ...
    with open('circling_log.txt', 'a') as f:
        f.write("Time,Distance,Bank,Error_Distance,Heading_Error,Tangent_Direction,Bearing_to_Plane,Yaw,Heading_Delta,Direction_Confirmed,Speed,Circle_Direction,Direction_Multiplier\n")
    direction_confirmed = False
    last_heading_sample_time = circling_start_time
    last_yaw = None
    while current_alt > IDEAL_HEIGHT or current_speed > IDEAL_SPEED:
        ...
        # Heading error: Angle to align with tangent
        heading_error = (tangent_direction - current_yaw + 180) % 360 - 180

        # Collect heading samples for direction confirmation
        if not direction_confirmed and current_time - circling_start_time <= DIRECTION_CONFIRM_TIME:
            if current_time - last_heading_sample_time >= HEADING_SAMPLE_INTERVAL:
                heading_samples.append((current_time - circling_start_time, current_yaw))
                last_heading_sample_time = current_time
                # Check direction after collecting enough samples
                if len(heading_samples) >= HEADING_SAMPLES:
                    detected_direction = confirm_circle_direction(heading_samples)
                    if detected_direction:
                        if detected_direction != circle_direction:
                            print(f"Direction corrected from {circle_direction} to {detected_direction}")
                            circle_direction = detected_direction
                            direction_multiplier = 1 if circle_direction == 'CW' else -1
                        else:
                            print(f"Direction confirmed: {circle_direction}")
                        direction_confirmed = True
                    else:
                        print(f"Direction confirmation inconclusive, retaining {circle_direction}")
                        direction_confirmed = True  # Proceed anyway

        # Bank angle control
        if direction_confirmed or current_time - circling_start_time > DIRECTION_CONFIRM_TIME:
            # Full control with distance correction
            error_distance = distance - CIRCLE_RADIUS
            desired_bank = (K_HEADING * heading_error + K_DISTANCE * error_distance) * direction_multiplier
        else:
            # Heading-only control during delay
            error_distance = 0.0  # For logging
            desired_bank = K_HEADING * heading_error * direction_multiplier

        desired_bank = max(min(desired_bank, MAX_ROLL), -MAX_ROLL)

        # Smooth bank transition
        desired_bank = 0.9 * last_bank + 0.1 * desired_bank
        ...
        # Calculate heading delta for logging
        heading_delta = 0.0
        if last_yaw is not None:
            delta_yaw = (current_yaw - last_yaw + 360) % 360
            heading_delta = delta_yaw if delta_yaw <= 180 else delta_yaw - 360
        last_yaw = current_yaw

        # Log data for analysis
        with open('circling_log.txt', 'a') as f:
            f.write(f"{current_time},{distance},{desired_bank},{error_distance},{heading_error},{tangent_direction},{bearing_to_plane},{current_yaw},{heading_delta},{1 if direction_confirmed else 0},{current_speed},{circle_direction},{direction_multiplier}\n")

        if current_time - last_print_time > 1:
            status = "Heading-only" if not direction_confirmed else "Full control"
            print(f"Circling ({status}) - Distance: {distance:.1f}m, Alt: {current_alt:.1f}m, Bank: {desired_bank:.1f}°, Heading Error: {heading_error:.1f}°, Direction: {circle_direction}")
