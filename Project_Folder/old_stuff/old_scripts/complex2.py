...
K_ROLL = 1.0              # Roll gain for glide and final approach
K_P = 0.1                 # Pitch gain
K_HEADING = 0.9           # Heading gain for normal circling
K_DISTANCE = 0.1          # Distance correction gain
...
K_HEADING_INITIAL = 1.5   # Heading gain for initial circling
MAX_CIRCLE_TIME = 300     # Max circling time (seconds)
MAX_INITIAL_CIRCLING_TIME = 30  # Max initial circling time (seconds)
ALIGNMENT_THRESHOLD = 10.0  # Heading error threshold (degrees)
CONSECUTIVE_CYCLES = 5      # Cycles for alignment confirmation
...
def get_start_location_input():
    ...
    while True:
        try:
            coord_input = input("Enter start location as 'lat,lon,alt': ")
...
def get_waypoint_input(vehicle):
    """Get landing waypoint from user."""
    while True:
        try:
            coord_input = input("Enter landing coordinates as 'lat,lon': ")
...
def glide_to_waypoint(vehicle, waypoint):
    ...
    while True:
        ...
        if distance <= IDEAL_RADIUS:
            ...
            direction_multiplier = -1  # CCW

            # Initial circling
            initial_circling_start_time = time.time()
            aligned_cycles = 0
            while True:
                set_throttle_off(vehicle)
                current_loc = vehicle.location.global_frame
                ...
                if aligned_cycles >= CONSECUTIVE_CYCLES:
                    logging.info("Heading aligned. Proceeding to normal circling.")
                    print("Heading aligned. Proceeding to normal circling.")
                    break

                if time.time() - initial_circling_start_time > MAX_INITIAL_CIRCLING_TIME:
                    logging.info("Initial circling timeout. Proceeding to normal circling.")
                    print("Initial circling timeout. Proceeding to normal circling.")
                    break

                desired_bank = K_HEADING_INITIAL * heading_error * direction_multiplier
                ...
                actual_bank = degrees(vehicle.attitude.roll) if vehicle.attitude.roll is not None else 0
                logging.info(f"Initial circling - Desired bank: {desired_bank:.1f}°, Actual: {actual_bank:.1f}°")
                if time.time() - last_print_time > 0.5:
                    print(f"Initial circling - Dist: {distance:.1f}m, Alt: {current_alt:.1f}m, Error: {heading_error:.1f}°")
                    last_print_time = time.time()
                time.sleep(CONTROL_FREQ)

            break  # Proceed to normal circling
    ...
    while (current_alt > IDEAL_HEIGHT and current_speed > SAFE_AIRSPEED and
        ...
        desired_bank = -(K_HEADING * heading_error + K_DISTANCE * error_distance +
