...
DIRECTION_CONFIRM_TIME = 1.0  # Time to confirm direction and enable distance correction (seconds)
HEADING_SAMPLES = 3        # Number of heading samples for direction confirmation
HEADING_SAMPLE_INTERVAL = 0.3  # Seconds between heading samples
MIN_YAW_CHANGE = 5.0       # Minimum yaw change per sample (degrees)
...
def confirm_circle_direction(heading_samples):
    ...
    if len(heading_samples) < HEADING_SAMPLES:
        return None
    # Check if headings are consistently increasing (CCW) or decreasing (CW)
    increasing = sum(1 for i in range(1, len(heading_samples)) if 
                     ((heading_samples[i][1] - heading_samples[i-1][1] + 360) % 360) < 180 and
                     abs(heading_samples[i][1] - heading_samples[i-1][1]) > MIN_YAW_CHANGE)
    decreasing = sum(1 for i in range(1, len(heading_samples)) if 
                     ((heading_samples[i][1] - heading_samples[i-1][1] + 360) % 360) > 180 and
                     abs(heading_samples[i][1] - heading_samples[i-1][1]) > MIN_YAW_CHANGE)
    if increasing >= HEADING_SAMPLES - 1:
        return 'CCW'
    if decreasing >= HEADING_SAMPLES - 1:
        return 'CW'
    return None
...
def glide_to_waypoint(vehicle, waypoint):
    ...
    with open('circling_log.txt', 'a') as f:
        f.write("Time,Distance,Bank,Error_Distance,Heading_Error,Tangent_Direction,Bearing_to_Plane,Yaw,Direction_Confirmed,Speed,Circle_Direction,Direction_Multiplier\n")
    ...
    while current_alt > IDEAL_HEIGHT or current_speed > IDEAL_SPEED:
        ...
        if not direction_confirmed and current_time - circling_start_time <= DIRECTION_CONFIRM_TIME:
            if current_time - last_heading_sample_time >= HEADING_SAMPLE_INTERVAL:
                heading_samples.append((current_time, current_yaw))
        ...
        with open('circling_log.txt', 'a') as f:
            f.write(f"{current_time},{distance},{desired_bank},{error_distance},{heading_error},{tangent_direction},{bearing_to_plane},{current_yaw},{1 if direction_confirmed else 0},{current_speed},{circle_direction},{direction_multiplier}\n")
