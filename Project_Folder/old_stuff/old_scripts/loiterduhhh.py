...
IDEAL_SPEED = 10.0        # Speed to stop circling (m/s, unused for breakout)
CIRCLE_RADIUS = 50.0     # Desired loiter radius (meters)
K_ROLL = 1.0              # Roll proportional gain for glide phase
K_P = 0.1                 # Pitch proportional gain
K_HEADING = 0.6           # Heading error gain (unused in loiter)
...
def glide_to_waypoint(vehicle, waypoint):
    ...
    with open('glide_log.txt', 'a') as f:
        f.write("Time,Distance,Desired_Roll,Actual_Roll,Pitch,Speed,Heading,Bearing\n")
    while True:
        current_loc = vehicle.location.global_frame
        current_alt = vehicle.location.global_relative_frame.alt
        ...
        if distance <= IDEAL_RADIUS:
            print(f"Within {IDEAL_RADIUS}m. Starting CCW loiter...")
            circle_direction = 'CCW'

            # Set loiter mode and parameters
            vehicle.mode = VehicleMode("LOITER")
            while vehicle.mode != "LOITER":
                time.sleep(1)

            # Set loiter radius and direction (CCW)
            vehicle.parameters['CIRCLE_RADIUS'] = CIRCLE_RADIUS * 100  # ArduPilot uses cm
            vehicle.parameters['CIRCLE_DIRECTION'] = 1  # 1 = CCW, -1 = CW

            # Command loiter at waypoint
            loiter_location = LocationGlobalRelative(waypoint.lat, waypoint.lon, current_alt)
            vehicle.simple_goto(loiter_location)

            # Loiter phase
            with open('circling_log.txt', 'a') as f:
                f.write("Time,Distance,Bank,Speed,Circle_Direction,Heading,Bearing_to_Plane\n")
            while current_alt > IDEAL_HEIGHT:
                ...
                # Estimate bank angle from attitude (roll)
                bank_angle = degrees(vehicle.attitude.roll) if vehicle.attitude is not None else 0.0

                # Log data
                with open('circling_log.txt', 'a') as f:
                    f.write(f"{time.time()},{distance},{bank_angle},{current_speed},{circle_direction},{current_yaw},{bearing_to_plane}\n")

                if time.time() - last_print_time > 0.5:
                    print(f"Circling - Distance: {distance:.1f}m, Alt: {current_alt:.1f}m, Bank: {bank_angle:.1f}°, Heading Error: {0.0:.1f}°, Speed: {current_speed:.1f}m/s, Heading: {current_yaw:.1f}°, Bearing: {bearing_to_plane:.1f}°, Direction: {circle_direction}")
                    last_print_time = time.time()

                time.sleep(CONTROL_FREQ)

            print("Loiter complete. Approaching waypoint...")
            break
        ...
        # Log glide data
        actual_roll = degrees(vehicle.attitude.roll) if vehicle.attitude is not None else 0.0
        with open('glide_log.txt', 'a') as f:
            f.write(f"{time.time()},{distance},{desired_roll},{actual_roll},{desired_pitch},{current_speed},{current_yaw},{bearing}\n")
...
def main():
    parser = argparse.ArgumentParser(description='Glide to waypoint with throttle off, loitering CCW.')
