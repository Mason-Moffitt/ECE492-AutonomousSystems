...
K_ROLL = 1.0              # Roll gain
K_ROLL_CIRCLE = 0.1       # Roll gain while circling
K_P = 0.1                 # Pitch gain
...
def glide_to_waypoint(vehicle, waypoint):
    ...
    while True:
        ...
        # Adjust roll for radius control based on inside/outside
        error_distance = distance - CIRCLE_RADIUS
        if error_distance > 0:
            roll_adjustment = -K_DISTANCE * error_distance  # Outside: negative roll to tighten
        else:
            roll_adjustment = K_DISTANCE * error_distance  # Inside: negative roll to widen

        desired_roll = -K_ROLL_CIRCLE * heading_error + roll_adjustment
        desired_roll = max(min(desired_roll, MAX_ROLL), -MAX_ROLL)
        ...
        if time.time() - last_print_time > 1:
            print(f"CCW Circling - Dist: {distance:.1f}m, Alt: {current_alt:.1f}m, Roll: {desired_roll:.1f}°")
            last_print_time = time.time()
        time.sleep(CONTROL_FREQ)
    ...
    while True:
        ...
        if time.time() - last_print_time > 1:
            print(f"Final Approach - Dist: {distance:.1f}m, Alt: {current_alt:.1f}m")
            last_print_time = time.time()
        time.sleep(CONTROL_FREQ)
