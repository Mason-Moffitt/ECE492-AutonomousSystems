...
HOME_LAT = 35.727312      # Fixed home and starting waypoint latitude
HOME_LON = -78.696101     # Fixed home and starting waypoint longitude
...
def send_mavlink_loiter_unlim(vehicle, lat, lon, alt, radius, clockwise=True):
    """Send MAV_CMD_NAV_LOITER_UNLIM to circle waypoint at specified radius."""
    msg = vehicle.message_factory.command_long_encode(
        0, 0,  # target system, component
        mavutil.mavlink.MAV_CMD_NAV_LOITER_UNLIM,  # command
        0,  # confirmation
        0,  # param1: not used
        radius if clockwise else -radius,  # param2: radius (positive=CW, negative=CCW)
        0,  # param3: xtrack (0=intersection exit)
        0,  # param4: yaw (not used)
        lat,  # param5: latitude
...
def get_start_location_input():
    """Get starting altitude for fixed home location (lat, lon)."""
    while True:
        try:
            alt = float(input("Enter starting altitude in meters (e.g., 50): "))
            if alt < MIN_START_ALT:
                print(f"Altitude must be at least {MIN_START_ALT}m.")
                continue
            return LocationGlobalRelative(HOME_LAT, HOME_LON, alt)
        except ValueError:
            print("Invalid format. Enter a number (e.g., 50).")
...
def glide_to_waypoint(vehicle, waypoint):
    """Control glide and circling to waypoint using MAVLink loiter command."""
    vehicle.mode = VehicleMode("GUIDED")
    while vehicle.mode != "GUIDED":
        time.sleep(1)
    ...
    while True:
        ...
        if distance <= IDEAL_RADIUS:
            ...
            clockwise = heading_error >= 0
            print(f"Within {IDEAL_RADIUS}m. Starting {'clockwise' if clockwise else 'counterclockwise'} circling.")
            break
    ...
    # Circling phase using MAV_CMD_NAV_LOITER_UNLIM
    set_throttle_off(vehicle)
    send_mavlink_loiter_unlim(
        vehicle,
        waypoint.lat,
        waypoint.lon,
        current_alt,  # Maintain current altitude
        CIRCLE_RADIUS,
        clockwise=clockwise
    )
    ...
    while True:
        ...
        if current_alt <= IDEAL_HEIGHT or current_speed <= IDEAL_SPEED:
            print("Circling complete. Approaching waypoint...")
            break

        if time.time() - last_print_time > 1:
            print(f"Circling - Distance: {distance:.1f}m, Alt: {current_alt:.1f}m, Speed: {current_speed:.1f}m/s")
            last_print_time = time.time()
