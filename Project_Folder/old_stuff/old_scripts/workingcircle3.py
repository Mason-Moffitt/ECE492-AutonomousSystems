...
K_ROLL = 0.8              # Roll proportional gain for glide phase
K_ROLL_CIRCLE = 0.2       # Roll gain for waypoint heading correction in circling phase
K_P = 0.1                 # Pitch proportional gain
K_HEADING = 0.6           # Heading error gain for tangent alignment
K_DISTANCE = 0.3          # Distance correction gain for bank angle
...
HORIZONTAL_TOLERANCE = 45.0  # Horizontal distance to consider start location reached (meters)
VERTICAL_TOLERANCE = 5.0    # Vertical distance to consider start altitude reached (meters)
# ArduPlane-specific constants
MAX_ROLL_INITIAL = 25.0   # Safer max roll for initial turn
K_HEADING_INITIAL = 1.5   # Aggressive heading gain for initial turn
SAFE_AIRSPEED = 7.0       # Minimum airspeed to prevent stall (adjust per plane)
...
def glide_to_waypoint(vehicle, waypoint):
    ...
    while True:
        ...
        if distance <= IDEAL_RADIUS:
            # Set provisional circling direction based on heading
            bearing = calculate_bearing(current_loc, waypoint)
            heading_error = (bearing - current_yaw + 180) % 360 - 180
            provisional_circle_direction = 'CW' if heading_error >= 0 else 'CCW'
            provisional_direction_multiplier = 1 if provisional_circle_direction == 'CW' else -1
            print(f"Within {IDEAL_RADIUS}m. Starting provisional {provisional_circle_direction} circling...")

            # Start timing for 4-second delay
            start_circle_time = time.time()
            circle_direction_determined = False
            circle_direction = provisional_circle_direction
            direction_multiplier = provisional_direction_multiplier
            velocities = []
            bearings = []  # Collect bearings for direction confirmation

            # Initial circling phase (4-second delay with aggressive alignment)
            while time.time() - start_circle_time < 4.0:
                set_throttle_off(vehicle)
                current_loc = vehicle.location.global_frame
                current_alt = vehicle.location.global_relative_frame.alt
                current_speed = vehicle.airspeed
                ...
                bearings.append(bearing_to_plane)

                # Calculate tangent direction based on provisional direction
                if provisional_circle_direction == 'CW':
                    tangent_direction = (bearing_to_plane - 90) % 360
                else:  # CCW
                    tangent_direction = (bearing_to_plane + 90) % 360

                # Heading error for aggressive alignment
                heading_error = (tangent_direction - current_yaw + 180) % 360 - 180

                # Adjust gain based on airspeed to prevent stall
                k_heading_current = K_HEADING_INITIAL if current_speed > SAFE_AIRSPEED else K_HEADING
                desired_bank = k_heading_current * heading_error * provisional_direction_multiplier
                desired_bank = max(min(desired_bank, MAX_ROLL_INITIAL), -MAX_ROLL_INITIAL)
                ...
                if time.time() - last_print_time > 0.5:
                    print(f"Initial circling - Distance: {distance:.1f}m, Alt: {current_alt:.1f}m, Bank: {desired_bank:.1f}°, Heading Error: {heading_error:.1f}°, Speed: {current_speed:.1f}m/s, Bearing: {bearing_to_plane:.1f}°, Direction: {provisional_circle_direction}")
                    last_print_time = time.time()

                time.sleep(CONTROL_FREQ)

            # Determine final circling direction using velocity and bearing changes
            v_avg = [sum(v[i] for v in velocities)/len(velocities) for i in range(2)]
            velocity_magnitude = sqrt(v_avg[0]**2 + v_avg[1]**2)
            bearing_to_plane = calculate_bearing(waypoint, current_loc)
            bearing_change = (bearings[-1] - bearings[0] + 360) % 360
            bearing_rate = bearing_change / 4.0  # Degrees per second

            if velocity_magnitude > 1.0:
                v_unit = [v_avg[0]/velocity_magnitude, v_avg[1]/velocity_magnitude]
                cw_tangent = (bearing_to_plane - 90) % 360
                ccw_tangent = (bearing_to_plane + 90) % 360
                cw_unit = [sin(radians(cw_tangent)), cos(radians(cw_tangent))]
                ccw_unit = [sin(radians(ccw_tangent)), cos(radians(ccw_tangent))]
                cw_dot = v_unit[0] * cw_unit[0] + v_unit[1] * cw_unit[1]
                ccw_dot = v_unit[0] * ccw_unit[0] + v_unit[1] * ccw_unit[1]
                
                # Use bearing rate as primary indicator
                if bearing_rate > 5.0:  # Increasing bearing indicates CCW
                    circle_direction = 'CCW'
                    print(f"Bearing-based direction: CCW (Bearing change: {bearing_change:.1f}° over 4s, Rate: {bearing_rate:.1f}°/s)")
                elif bearing_rate < -5.0:  # Decreasing bearing indicates CW
                    circle_direction = 'CW'
                    print(f"Bearing-based direction: CW (Bearing change: {bearing_change:.1f}° over 4s, Rate: {bearing_rate:.1f}°/s)")
                else:  # Fallback to velocity if bearing change is small
                    circle_direction = 'CW' if cw_dot >= ccw_dot else 'CCW'
                    print(f"Velocity-based direction: {circle_direction} (CW dot: {cw_dot:.2f}, CCW dot: {ccw_dot:.2f}, Bearing change: {bearing_change:.1f}°)")
            else:
                # Use bearing rate if velocity is too small
                if bearing_rate > 5.0:
                    circle_direction = 'CCW'
                    print(f"Bearing-based direction: CCW (Low velocity, Bearing change: {bearing_change:.1f}° over 4s, Rate: {bearing_rate:.1f}°/s)")
                elif bearing_rate < -5.0:
                    circle_direction = 'CW'
                    print(f"Bearing-based direction: CW (Low velocity, Bearing change: {bearing_change:.1f}° over 4s, Rate: {bearing_rate:.1f}°/s)")
                else:
                    circle_direction = provisional_circle_direction
                    print(f"Velocity too small and bearing change small, keeping provisional direction: {circle_direction}")

            direction_multiplier = 1 if circle_direction == 'CW' else -1
            print(f"Final circling direction set to {circle_direction}.")
            break
    ...
    with open('circling_log.txt', 'a') as f:
        f.write("Time,Distance,Bank,Error_Distance,Heading_Error,Tangent_Direction,Speed,Circle_Direction,Velocity_N,Velocity_E,Bearing_to_Plane\n")
    while current_alt > IDEAL_HEIGHT or current_speed > IDEAL_SPEED:
        ...
        # Heading errors
        heading_error = (tangent_direction - current_yaw + 180) % 360 - 180
        waypoint_heading_error = (bearing - current_yaw + 180) % 360 - 180

        # Distance error
        error_distance = distance - CIRCLE_RADIUS

        # Combine heading, distance, and waypoint heading corrections
        desired_bank = (K_HEADING * heading_error + K_DISTANCE * error_distance + K_ROLL_CIRCLE * waypoint_heading_error) * direction_multiplier
        ...
        # Log data
        with open('circling_log.txt', 'a') as f:
            f.write(f"{time.time()},{distance},{desired_bank},{error_distance},{heading_error},{tangent_direction},{current_speed},{circle_direction},{velocity[0]},{velocity[1]},{bearing_to_plane}\n")

        if time.time() - last_print_time > 0.5:
            print(f"Circling - Distance: {distance:.1f}m, Alt: {current_alt:.1f}m, Bank: {desired_bank:.1f}°, Heading Error: {heading_error:.1f}°, Speed: {current_speed:.1f}m/s, Bearing: {bearing_to_plane:.1f}°, Direction: {circle_direction}")
            last_print_time = time.time()
