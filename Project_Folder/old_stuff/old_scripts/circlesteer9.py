...
K_ROLL = 0.7              # Roll proportional gain for glide phase
...
BASE_BANK = 10.0          # Fixed bank angle for initial circling (degrees, positive = right)
...
def glide_to_waypoint(vehicle, waypoint):
    """Control glide and circling to waypoint with fixed bank initial control."""
    vehicle.mode = VehicleMode("GUIDED")
    while vehicle.mode != "GUIDED":
        time.sleep(1)
    ...
    while True:
        ...
        if distance <= IDEAL_RADIUS:
            print(f"Within {IDEAL_RADIUS}m. Starting circling with fixed bank control.")
    ...
    while current_alt > IDEAL_HEIGHT or current_speed > IDEAL_SPEED:
        ...
        if circle_direction is None:
            # Initial fixed bank with distance correction (first 2 seconds)
            error_distance = distance - CIRCLE_RADIUS
            desired_bank = BASE_BANK - K_DISTANCE * error_distance  # Positive BASE_BANK = right bank
        ...
        if current_time - last_print_time > 1:
            status = "Initial circling (fixed bank)" if circle_direction is None else "Full control"
