...
K_DISTANCE = 0.3          # Distance correction gain for bank angle (secondary)
...
K_HEADING_INITIAL = 1.2   # Aggressive heading gain for initial turn
SAFE_AIRSPEED = 8.0       # Minimum airspeed to prevent stall (adjust per plane)
...
def glide_to_waypoint(vehicle, waypoint):
    ...
    while True:
        ...
        if distance <= IDEAL_RADIUS:
            ...
            # Start timing for 1-second delay
            ...
            velocities = []  # Collect velocities for averaging

            # Initial circling phase (1-second delay with aggressive alignment)
            while time.time() - start_circle_time < 4.0:
                ...
                if time.time() - last_print_time > 1:
                    print(f"Initial circling - Distance: {distance:.1f}m, Alt: {current_alt:.1f}m, Bank: {desired_bank:.1f}°, Heading Error: {heading_error:.1f}°, Provisional Direction: {provisional_circle_direction}")
                    last_print_time = time.time()

                time.sleep(CONTROL_FREQ)

            # Determine final circling direction using averaged velocity
            ...
            if velocity_magnitude > 1.0:
                ...
                print(f"Velocity-based direction after 1s: {circle_direction} (CW dot: {cw_dot:.2f}, CCW dot: {ccw_dot:.2f})")
            else:
                circle_direction = provisional_circle_direction
                print("Velocity too small, keeping provisional direction.")
    ...
    while current_alt > IDEAL_HEIGHT or current_speed > IDEAL_SPEED:
        ...
        # Combine heading and distance corrections
        ...
        if time.time() - last_print_time > 1:
            print(f"Circling - Distance: {distance:.1f}m, Alt: {current_alt:.1f}m, Bank: {desired_bank:.1f}°, Heading Error: {heading_error:.1f}°, Direction: {circle_direction}")
            last_print_time = time.time()
