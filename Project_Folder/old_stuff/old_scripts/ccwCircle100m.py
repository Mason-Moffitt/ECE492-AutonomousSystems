...
IDEAL_HEIGHT = 24.0       # Height to stop circling (meters)
...
K_ROLL_CIRCLE = 0.075      # Roll gain for waypoint heading correction in circling phase
K_P = 0.1                 # Pitch proportional gain
K_HEADING = 0.4           # Heading error gain for tangent alignment .6works
K_DISTANCE = 0.01          # Distance correction gain for bank angle .2works
...
def get_start_location_input():
    ...
    while True:
        try:
            coord_input = input("Enter starting location as 'lat,lon,alt' (e.g. SW: 35.726010,-78.697287,200): ")
...
def glide_to_waypoint(vehicle, waypoint):
    ...
    while True:
        ...
        actual_descent_rate = vehicle.velocity[2] if vehicle is not None and vehicle.velocity is not None else 0
        distance = get_distance_metres(current_loc, waypoint)
        ...
        if distance <= IDEAL_RADIUS:
            ...
            direction_multiplier = -1  # Negative for CCW (left turn)

            # Initial circling phase (4-second transition to CCW)
            ...
            while time.time() - start_circle_time < 4.0:
                ...
                # CCW tangent direction (90° left of bearing)
                ...
                if time.time() - last_print_time > 0.5:
                    print(f"Initial CCW circling - Distance: {distance:.1f}m, Alt: {current_alt:.1f}m, Bank: {desired_bank:.1f}°, Heading Error: {heading_error:.1f}°, Speed: {current_speed:.1f}m/s, Heading: {current_yaw:.1f}°, Bearing: {bearing_to_plane:.1f}°")
                    last_print_time = time.time()

                time.sleep(CONTROL_FREQ)

            print("Transition complete. Continuing CCW circling.")
            break
    ...
    while current_alt > IDEAL_HEIGHT or current_speed > IDEAL_SPEED:
        ...
        # Heading errors
        heading_error = (tangent_direction - current_yaw + 180) % 360 - 180
        bearing = calculate_bearing(current_loc, waypoint)  # Waypoint bearing
        ...
        desired_bank = -(K_HEADING * heading_error + K_DISTANCE * error_distance + K_ROLL_CIRCLE * waypoint_heading_error)  # Negative for CCW
    ...
    while True:
        ...
        current_yaw = vehicle.heading
        distance = get_distance_metres(current_loc, waypoint)
        vertical_speed = vehicle.velocity[2] if vehicle is not None and vehicle.velocity is not None else 0
...
def main():
    parser = argparse.ArgumentParser(description='Glide to waypoint with throttle off, always circling CCW.')
