...
def wait_for_manual_arm(vehicle):
    """Wait for the vehicle to be manually armed by checking HEARTBEAT messages."""
    ...
    while True:
        ...
        if msg is None:
            ...
            continue
        # Check if the vehicle is armed (MAV_MODE_FLAG_SAFETY_ARMED = 128)
...
def display_telemetry(vehicle, target_lat, target_lon):
    ...
    while telemetry_running:
        # Get airspeed (VFR_HUD provides airspeed in m/s)
        ...
        # Get altitude AGL (using relative altitude from GLOBAL_POSITION_INT)
        msg_pos = vehicle.recv_match(type='GLOBAL_POSITION_INT', blocking=True, timeout=1)
        if msg_pos:
            current_altitude_agl = msg_pos.relative_alt / 1000.0  # Convert mm to meters
        else:
            current_altitude_agl = 0.0

        # Calculate distance to waypoint
        if msg_pos and hasattr(vehicle, 'location') and vehicle.location.global_relative_frame:
...
def main():
    ...
    try:
        ...
        while vehicle.mode.name != "TAKEOFF" and time.time() < timeout:
            time.sleep(1)
