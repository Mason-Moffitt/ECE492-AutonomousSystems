...
START_CIRCLE_RADIUS = 100.0 # Distance to start circling (meters)
...
K_ROLL_FINAL = 1.0          # Increased roll gain for final approach
K_P = 0.1                   # Pitch gain
K_HEADING = 0.0             # Reduced heading gain for circling
K_DISTANCE = 0.4            # Reduced distance correction gain
K_WIND = 0.0                # Wind correction gain
...
def glide_to_waypoint(vehicle, waypoint):
    ...
    alpha_bank = 0.95  # Bank smoothing factor
    ...
    # Circling phase (exit based solely on altitude)
    while current_alt > IDEAL_HEIGHT:
        ...
        desired_bank_centripetal = degrees(atan2(filtered_speed**2, g * CIRCLE_RADIUS))

        # Bank angle with reduced gains and increased smoothing
