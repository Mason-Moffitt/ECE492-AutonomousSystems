...
IDEAL_RADIUS = 200.0      # Distance to start circling (meters)
...
K_DISTANCE = 0.6          # Distance correction gain for bank angle
...
HORIZONTAL_TOLERANCE = 30.0  # Horizontal distance to consider start location reached (meters)
VERTICAL_TOLERANCE = 5.0    # Vertical distance to consider start altitude reached (meters)
LOITER_TIME = 10.0        # Time to loiter at start location (seconds)
...
def navigate_to_start_location(vehicle, start_location):
    """Navigate to the starting location and loiter."""
...
def get_waypoint_input(vehicle):
    """Get landing waypoint and circling direction from user and validate altitude."""
    while True:
        try:
    ...
    # Prompt for circling direction
    while True:
        direction = input("Enter circling direction (CW for clockwise, CCW for counterclockwise): ").strip().upper()
        if direction in ['CW', 'CCW']:
            return waypoint, direction
        print("Invalid input. Enter 'CW' or 'CCW'.")

def glide_to_waypoint(vehicle, waypoint, circle_direction):
    """Control glide and circling to waypoint."""
    vehicle.mode = VehicleMode("GUIDED")
    while vehicle.mode != "GUIDED":
        time.sleep(1)
    ...
    # Set direction multiplier: +1 for CW, -1 for CCW
    ...
    while True:
        ...
        if current_alt < 2.2 and abs(vertical_speed) < 0.1:
            ...
            print(f"Landing detected. Disarming vehicle.")
            vehicle.armed = False
    ...
    while vehicle.mode != "MANUAL":
        time.sleep(1)
    print("Mode set to MANUAL. Ready to Disarmed.")
...
def main():
    ...
    waypoint, circle_direction = get_waypoint_input(vehicle)
    glide_to_waypoint(vehicle, waypoint, circle_direction)
