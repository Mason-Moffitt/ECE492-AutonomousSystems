...
K_ROLL = 2.0              # Roll gain for glide phase
K_ROLL_FINAL = 2.0        # Increased roll gain for final approach
K_P = 0.1                 # Pitch gain
K_HEADING = 0.4           # Reduced heading gain for circling
K_DISTANCE = 0.05         # Proportional gain for distance PID
K_I_DISTANCE = 0.005      # Integral gain for distance PID
K_D_DISTANCE = 0.03       # Derivative gain for distance PID
K_WIND = 0.05             # Wind correction gain
THROTTLE_MIN = 1000       # PWM for throttle off
CONTROL_FREQ = 0.05       # Control loop frequency (20 Hz)
MAX_ROLL = 35.0           # Reduced max roll angle (degrees)
...
alpha_bank = 0.98         # Bank smoothing factor
...
def glide_to_waypoint(vehicle, waypoint):
    ...
    while current_alt > IDEAL_HEIGHT:
        ...
        distance_correction = (K_DISTANCE * error_distance +
                               K_I_DISTANCE * integral_distance_error +
                               K_D_DISTANCE * derivative_distance_error)

        # Feedforward term: ideal bank angle for circular flight
        ideal_bank = degrees(atan2(filtered_speed**2, g * CIRCLE_RADIUS))
