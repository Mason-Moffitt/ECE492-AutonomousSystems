...
K_ROLL = 0.65              # Roll proportional gain for glide phase
K_P = 0.1                 # Pitch proportional gain
K_HEADING = 0.5           # Heading error gain for tangent alignment
K_DISTANCE = 0.45          # Distance correction gain for bank angle (secondary)
...
DIRECTION_CHECK_TIME = 5.0  # Time to check direction after circling entry (seconds)
BEARING_SAMPLES = 3        # Number of bearing samples for direction confirmation
BEARING_INTERVAL = 1.0     # Seconds between bearing samples
MIN_BEARING_CHANGE = 5.0   # Minimum bearing change per sample (degrees)
...
def confirm_circle_direction(waypoint, bearing_samples):
    """Confirm circling direction (CW/CCW) based on bearing samples."""
    if len(bearing_samples) < BEARING_SAMPLES:
        return None
    # Check if bearings are consistently increasing (CCW) or decreasing (CW)
    increasing = sum(1 for i in range(1, len(bearing_samples)) if 
                     ((bearing_samples[i][1] - bearing_samples[i-1][1] + 360) % 360) < 180 and
                     abs(bearing_samples[i][1] - bearing_samples[i-1][1]) > MIN_BEARING_CHANGE)
    decreasing = sum(1 for i in range(1, len(bearing_samples)) if 
                     ((bearing_samples[i][1] - bearing_samples[i-1][1] + 360) % 360) > 180 and
                     abs(bearing_samples[i][1] - bearing_samples[i-1][1]) > MIN_BEARING_CHANGE)
    if increasing >= BEARING_SAMPLES - 1:
        return 'CCW'
    if decreasing >= BEARING_SAMPLES - 1:
        return 'CW'
    return None
...
def glide_to_waypoint(vehicle, waypoint):
    """Control glide and circling to waypoint using tangent alignment and distance correction."""
    vehicle.mode = VehicleMode("GUIDED")
    while vehicle.mode != "GUIDED":
        time.sleep(1)
    ...
    with open('circling_log.txt', 'a') as f:
        f.write("Time,Distance,Bank,Error_Distance,Heading_Error,Tangent_Direction,Bearing_to_Plane,Speed,Circle_Direction,Direction_Multiplier\n")
    direction_confirmed = False
    last_bearing_sample_time = circling_start_time
    while current_alt > IDEAL_HEIGHT or current_speed > IDEAL_SPEED:
        ...
        # Collect bearing samples for direction confirmation
        if not direction_confirmed and current_time - circling_start_time >= 2.0:
            if current_time - last_bearing_sample_time >= BEARING_INTERVAL:
                bearing_to_plane = calculate_bearing(waypoint, current_loc)
                bearing_samples.append((current_time, bearing_to_plane))
                last_bearing_sample_time = current_time
                # Check direction after collecting enough samples
                if len(bearing_samples) >= BEARING_SAMPLES:
                    detected_direction = confirm_circle_direction(waypoint, bearing_samples)
                    if detected_direction and detected_direction != circle_direction:
        ...
        # Distance error
        error_distance = distance - CIRCLE_RADIUS  # Positive if too far, negative if too close
        ...
        with open('circling_log.txt', 'a') as f:
            f.write(f"{current_time},{distance},{desired_bank},{error_distance},{heading_error},{tangent_direction},{bearing_to_plane},{current_speed},{circle_direction},{direction_multiplier}\n")
