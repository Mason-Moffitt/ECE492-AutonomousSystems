...
def glide_to_waypoint(vehicle, waypoint):
        ...
        while True:
            ...
            bearing = calculate_bearing(current_loc, waypoint)
            desired_heading = (bearing + 90) % 360  # Base CCW heading
            e_a = (desired_heading - current_yaw + 180) % 360 - 180  # Angular error
        
            # Calculate weights
            THRESHOLD = 20  # Meters, tune this value
            w_d = min(1, abs(e_d) / THRESHOLD)  # Distance weight, proportional to |e_d|
            w_a = 1 - w_d  # Angular weight, inversely proportional
        
            # Calculate desired roll
            K_d = 0.8  # Distance gain, tune this
            K_a = 1.0  # Angular gain, tune this
            distance_roll = K_d * e_d * w_d  # Distance correction component
            angular_roll = K_a * e_a * w_a  # Angular correction component
            desired_roll = distance_roll + angular_roll
            desired_roll = max(min(desired_roll, MAX_ROLL), -MAX_ROLL)
            ...
            # Debug output
            if time.time() - last_print_time > 1:
                print(f"Circling - Dist: {distance:.1f}m, e_d: {e_d:.1f}m, w_d: {w_d:.2f}, e_a: {e_a:.1f}°, Roll: {desired_roll:.1f}°, Pitch: {desired_pitch:.1f}°")
                last_print_time = time.time()
            time.sleep(CONTROL_FREQ)
