...
def glide_to_waypoint(vehicle, waypoint):
    ...
    while True:
        ...
        if distance <= IDEAL_RADIUS:
            print(f"Within {IDEAL_RADIUS}m. Starting initial circling to determine direction...")

            # Start timing for 4-second wait
            start_circle_time = time.time()
            headings = []  # Collect headings for direction determination
            bearings = []  # Collect bearings for logging

            # Initial circling phase (4-second wait to observe heading change)
            while time.time() - start_circle_time < 4.0:
                ...
                # Bank toward waypoint bearing (neutral initial alignment)
                heading_error = (bearing_to_plane - current_yaw + 180) % 360 - 180
                k_heading_current = K_HEADING_INITIAL if current_speed > SAFE_AIRSPEED else K_HEADING
                desired_bank = k_heading_current * heading_error
                ...
                if time.time() - last_print_time > 0.5:
                    print(f"Initial circling - Distance: {distance:.1f}m, Alt: {current_alt:.1f}m, Bank: {desired_bank:.1f}°, Heading Error: {heading_error:.1f}°, Speed: {current_speed:.1f}m/s, Heading: {current_yaw:.1f}°, Bearing: {bearing_to_plane:.1f}°")
                    last_print_time = time.time()

                time.sleep(CONTROL_FREQ)

            # Determine circling direction using heading change
            heading_change = (headings[-1] - headings[0] + 360) % 360
            if heading_change > 180:
                heading_change -= 360  # Normalize to [-180°, 180°]
            heading_rate = heading_change / 4.0  # Degrees per second
            bearing_change = (bearings[-1] - bearings[0] + 360) % 360 - 180  # For logging

            if abs(heading_rate) > 5.0:  # Significant heading change
                if heading_rate > 0:  # Increasing heading (right turn)
                    circle_direction = 'CW'
                    print(f"Heading-based direction: CW (Heading change: {heading_change:.1f}° over 4s, Rate: {heading_rate:.1f}°/s, Bearing change: {bearing_change:.1f}°)")
                else:  # Decreasing heading (left turn)
                    circle_direction = 'CCW'
                    print(f"Heading-based direction: CCW (Heading change: {heading_change:.1f}° over 4s, Rate: {heading_rate:.1f}°/s, Bearing change: {bearing_change:.1f}°)")
            else:  # Small heading change, default to CCW
                circle_direction = 'CCW'
                print(f"Heading change too small, defaulting to CCW (Heading change: {heading_change:.1f}° over 4s, Rate: {heading_rate:.1f}°/s, Bearing change: {bearing_change:.1f}°)")
    ...
    with open('circling_log.txt', 'a') as f:
        f.write("Time,Distance,Bank,Error_Distance,Heading_Error,Tangent_Direction,Speed,Circle_Direction,Heading,Bearing_to_Plane\n")
    while current_alt > IDEAL_HEIGHT or current_speed > IDEAL_SPEED:
        ...
        # Calculate tangent direction based on determined direction
        ...
        # Log data
        with open('circling_log.txt', 'a') as f:
            f.write(f"{time.time()},{distance},{desired_bank},{error_distance},{heading_error},{tangent_direction},{current_speed},{circle_direction},{current_yaw},{bearing_to_plane}\n")

        if time.time() - last_print_time > 0.5:
            print(f"Circling - Distance: {distance:.1f}m, Alt: {current_alt:.1f}m, Bank: {desired_bank:.1f}°, Heading Error: {heading_error:.1f}°, Speed: {current_speed:.1f}m/s, Heading: {current_yaw:.1f}°, Bearing: {bearing_to_plane:.1f}°, Direction: {circle_direction}")
            last_print_time = time.time()
