...
DESCENT_PITCH = -5.0      # Desired pitch for descent (degrees)
...
def get_waypoint_input(vehicle):
    """Get and validate waypoint within gliding range."""
    while True:
        ...
        try:
            ...
            if distance > max_glide_distance:
                print(f"Waypoint {distance:.1f}m exceeds glide range {max_glide_distance:.1f}m.")
                continue
            print(f"Waypoint accepted: {distance:.1f}m, within {max_glide_distance:.1f}m.")
            return waypoint
        except ValueError:
...
def glide_to_waypoint(vehicle, waypoint):
    ...
    while current_alt > IDEAL_HEIGHT or current_speed > IDEAL_SPEED:
        ...
        circle_bearing = (angle + current_yaw) % 360
        dlat = (CIRCLE_RADIUS * cos(radians(circle_bearing))) / 111320
        dlon = (CIRCLE_RADIUS * sin(radians(circle_bearing))) / (111320 * cos(radians(waypoint.lat)))
        circle_point = LocationGlobal(waypoint.lat + dlat, waypoint.lon + dlon, 0)

        bearing = calculate_bearing(current_loc, circle_point)
        heading_error = (bearing - current_yaw + 180) % 360 - 180
        desired_roll = K_ROLL * heading_error
        desired_pitch = DESCENT_PITCH
        send_attitude_target(vehicle, desired_roll, desired_pitch, current_yaw)
